<template>
  <div class="mx- w-80 rounded-md border border-gray-400 p-4 shadow-lg">
    <div class="flex w-full justify-center">
      <p class="font-sans text-2xl tracking-widest">Registrieren</p>
    </div>
...
const { registerUser } = useFirebaseAuth(); //Custom composable
...
async function handleRegistration() {
  const authStatus = await registerUser(creds.email, creds.password);
  if (authStatus) {
    toast.add({
      severity: 'success',
      summary: 'Erfolgreich!',
